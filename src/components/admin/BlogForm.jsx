...
const BlogForm = ({ isEdit = false }) => {
  const [formData, setFormData] = useState({
    ...
    is_featured: false,
    featured_image: null,
    current_image: '', // New field to store existing image URL
  });
      ...
      const fetchBlog = async () => {
        try {
          ...
          setFormData({
            ...
            categories: Array.isArray(blogData.category) ? blogData.category.join(',') : blogData.category || '',
            tags: Array.isArray(blogData.tags) ? blogData.tags.join(',') : blogData.tags || '',
            meta_description: blogData.meta_description || '',
            is_featured: !!blogData.is_featured,
            featured_image: null,
            current_image: blogData.image || '', // Store existing image URL
          });
        } catch (err) {
  ...
  const handleSubmit = async (e) => {
    ...
    if (formData.featured_image) {
      ...
      data.append('image', formData.current_image); // Send existing image URL if no new image is uploaded
    }
